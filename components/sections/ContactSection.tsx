...
import { Mail, MapPin, Briefcase, Send, CheckCircle2, Linkedin } from "lucide-react";
import { FaGithub } from "react-icons/fa";
import { fadeUp, stagger } from "@/lib/animations";
import { useReveal } from "@/lib/hooks/useReveal";

type FormData = { name: string; email: string; subject: string; message: string; _trap: string };
type FormStatus = "idle" | "loading" | "success" | "error";

const CONTACT_INFO = [
  { icon: Mail,      label: "Email",    value: "sameer.shaikh0425@gmail.com",          href: "mailto:sameer.shaikh0425@gmail.com", color: "#00D4FF" },
  { icon: MapPin,    label: "Location", value: "Mumbai, Maharashtra, India",            href: null,                                 color: "#00FF88" },
  { icon: Briefcase, label: "Role",     value: "Cybersecurity Engineer & GRC Analyst", href: null,                                 color: "#8B5CF6" },
];

export default function ContactSection() {
  const { ref, inView } = useReveal();
  const [form,   setForm]   = useState<FormData>({ name: "", email: "", subject: "", message: "", _trap: "" });
  ...
  return (
    <section id="contact" aria-label="Contact" className="section-main relative overflow-hidden">
      <div className="absolute inset-0 cyber-grid-bg pointer-events-none opacity-30" aria-hidden="true" />
      <div className="absolute top-0 right-0 w-[500px] h-[400px] pointer-events-none" aria-hidden="true"
      ...
      <div className="section-inner relative z-10">
        <m.div ref={ref} variants={stagger()} initial="hidden" animate={inView ? "visible" : "hidden"}>
          <m.p variants={fadeUp} className="section-label mb-3">Get in Touch</m.p>
          ...
          <m.p variants={fadeUp} className="section-subtitle mb-10">
            Open to cybersecurity roles, GRC consulting, and full-stack development opportunities.
          </m.p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12">
            ...
            <m.div variants={fadeUp} className="lg:col-span-2 space-y-6">
              {CONTACT_INFO.map(({ icon: Icon, label, value, href, color }) => (
                <div key={label} className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-xl border flex items-center justify-center flex-shrink-0"
                    style={{ background: `${color}0D`, borderColor: `${color}25` }} aria-hidden="true">
                    <Icon className="w-4 h-4" style={{ color }} />
                  </div>
                  <div>
                    <p className="font-mono-jet text-[10px] uppercase tracking-wider text-[#2E4560] mb-0.5">{label}</p>
                    {href ? (
                      <a href={href} className="font-inter-var text-sm text-[#C4DCF0] hover:text-[#00D4FF] transition-colors break-all">{value}</a>
                    ) : (
                      <p className="font-inter-var text-sm text-[#C4DCF0]">{value}</p>
                    )}
                  </div>
                </div>
              ))}
              ...
              <div className="pt-4 border-t border-[#162030]">
                <p className="font-mono-jet text-[10px] uppercase tracking-wider text-[#2E4560] mb-3">// Profiles</p>
              ...
              <div className="flex items-center gap-2 px-4 py-3 rounded-xl border border-[#00FF88]/20 bg-[#00FF88]/04">
                <span className="w-2 h-2 rounded-full bg-[#00FF88] animate-pulse flex-shrink-0" aria-hidden="true" />
                <span className="font-mono-jet text-xs text-[#00FF88]">Available for new opportunities</span>
              </div>
            </m.div>

            {/* Form */}
            <m.div variants={fadeUp} className="lg:col-span-3">
              <div className="rounded-xl border p-6 sm:p-8 relative"
                ...
                <div className="flex items-center gap-2 mb-6 pb-4 border-b border-[#162030]">
                  ...
                  <span className="font-mono-jet text-xs text-[#2E4560]">$ send_message.sh</span>
                </div>
                ...
                {status === "success" ? (
                  <m.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div>
                      <p className="font-pliant text-xl font-bold text-[#00FF88] mb-2">Message Sent!</p>
                      <p className="font-inter-var text-sm text-[#6B8EAD]">I&apos;ll get back to you within 24 hours.</p>
                    </div>
                    <button onClick={() => { setStatus("idle"); setForm({ name: "", email: "", subject: "", message: "", _trap: "" }); }}
                  ...
                  <form onSubmit={handleSubmit} noValidate aria-label="Contact form">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <input id="contact-name" name="name" type="text" required
                          ...
                          autoComplete="name" />
                        {errors.name && <p id="err-name" className="mt-1 font-mono-jet text-xs text-[#FF3B5C]" role="alert">{errors.name}</p>}
                      </div>
                      <div>
                        ...
                        <input id="contact-email" name="email" type="email" required
                          ...
                          autoComplete="email" />
                        {errors.email && <p id="err-email" className="mt-1 font-mono-jet text-xs text-[#FF3B5C]" role="alert">{errors.email}</p>}
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      ...
                      <input id="contact-subject" name="subject" type="text" required
                        value={form.subject} onChange={handleChange} placeholder="Regarding cybersecurity consultation..."
                      ...
                      {errors.subject && <p id="err-subject" className="mt-1 font-mono-jet text-xs text-[#FF3B5C]" role="alert">{errors.subject}</p>}
                    </div>

                    <div className="mb-5">
                      <label htmlFor="contact-message" className="form-label">Message</label>
                      <textarea id="contact-message" name="message" required rows={5}
                        value={form.message} onChange={handleChange} placeholder="Write your message here..."
                      ...
                      {errors.message && <p id="err-message" className="mt-1 font-mono-jet text-xs text-[#FF3B5C]" role="alert">{errors.message}</p>}
                    </div>

                    {status === "error" && (
                      <p className="mb-4 font-mono-jet text-xs text-[#FF3B5C] text-center p-3 rounded-lg border border-[#FF3B5C]/20 bg-[#FF3B5C]/05" role="alert">
                    ...
                    <button type="submit" disabled={status === "loading"}
                      ...
                      {status === "loading" ? (
                        <><span className="animate-spin inline-block">&#8635;</span> Transmitting...</>
                      ) : (
